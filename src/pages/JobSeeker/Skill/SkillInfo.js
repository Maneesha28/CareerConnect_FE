...
function SkillInfo() {
  ...
  const handleDeleteSkillInfo = async (infoId) => {
    try {
      const response = await axios.delete(`http://localhost:3000/api/skill/${infoId}`, {
        headers: {
          'Content-Type': 'application/json',
        },
        withCredentials: true,
  ...
  const handleSaveSkillInfo = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:3000/api/skill", transformedData, {
        headers: {
          'Content-Type': 'application/json',
        },
        withCredentials: true,
  ...
  const handleSaveEdit = async () => {
    if (editedSkillInfo) {
      ...
      try {
        const response = await axios.put(`http://localhost:3000/api/skill/${editedSkillInfo.skill_id}`, transformedData, {
        headers: {
          'Content-Type': 'application/json',
        },
        withCredentials: true,
